...
function genCode() { return nanoid(6).toUpperCase(); }
...
function isSafeTileAbs(absIdx) { return SAFE_ABS.includes(absIdx); }

function allPiecesFinished(p) { return p.pieces.every(v => v === 57); }
...
wss.on('connection', ws => {
  ...
  ws.on('message', msg => {
    ...
    if (data.type === 'create_room') {
      ...
      room.hostId = ws.id;
      ws.roomCode = code;
      send(ws, 'room_created', { code, link: `${data.origin}/?room=${code}` });
    }
    ...
    if (data.type === 'join_room') {
      ...
      if (room.players.size >= 4) return send(ws, 'error', { message: 'Room is full' });

      const used = new Set(Array.from(room.players.values()).map(p => p.color));
      const color = ['red','green','yellow','blue'].find(c => !used.has(c));

      room.players.set(ws.id, { id: ws.id, ws, name: name || 'Player', color, state: makeInitialPlayerState() });
      ws.roomCode = code;

      startIfPossible(room);

      broadcast(room, { type: 'room_update', 
        players: Array.from(room.players.values()).map(p => ({ id: p.id, name: p.name, color: p.color })),
        started: room.started, code,
    ...
    if (data.type === 'start_game') {
      ...
      broadcast(room, { type: 'room_update',
        players: Array.from(room.players.values()).map(p => ({ id: p.id, name: p.name, color: p.color })),
        started: room.started, code: ws.roomCode, turn: room.state.turn, log: room.state.log });
    }

    if (data.type === 'roll_dice') {
      const room = rooms.get(ws.roomCode);
      if (!room || !room.started) return;
      if (room.state.turn !== ws.id) return send(ws, 'error', { message: 'Not your turn' });
      ...
      const movable = [];
      for (let i = 0; i < 4; i++) {
        if (canMovePiece(p.state.pieces[i], roll)) movable.push(i);
      }
    ...
    if (data.type === 'move_piece') {
      ...
      const room = rooms.get(ws.roomCode);
      if (!room || !room.started) return;
      if (room.state.turn !== ws.id) return;
      const roll = room.state.dice;
      if (!roll) return;

      const p = room.players.get(ws.id);
      const rel = p.state.pieces[pieceIndex];
      if (!canMovePiece(rel, roll)) return;
      ...
      if (allPiecesFinished(p)) {
        ...
        broadcast(room, { type: 'game_over', winner: { id: p.id, name: p.name, color: p.color } });
        return;
      }
      ...
      broadcast(room, { type: 'state', 
        code: ws.roomCode,
        players: Array.from(room.players.values()).map(pl => ({ id: pl.id, name: pl.name, color: pl.color, pieces: pl.state.pieces })),
        log: room.state.log
      });
  ...
  ws.on('close', () => {
    ...
    broadcast(room, { type: 'room_update', 
      players: Array.from(room.players.values()).map(p => ({ id: p.id, name: p.name, color: p.color })),
      started: room.started, code, turn: room.state.turn, log: room.state.log });
  });
});
